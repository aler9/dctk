...
func main() {
	...
	if hash != dctk.TigerHashMust("LWPNACQDBZRYXW3VHJVCJ64QBZNGHOHHHZWCLNQ") {
		panic(fmt.Errorf("wrong hash (3): %s", hash))
	}

	hash = dctk.TTHFromBytes([]byte("\x00"))
	if hash != dctk.TigerHashMust("VK54ZIEEVTWNAUI5D5RDFIL37LX2IQNSTAXFKSA") {
		panic(fmt.Errorf("wrong hash (4): %s", hash))
	}

	hash = dctk.TTHFromBytes([]byte(strings.Repeat("A", 1024)))
	if hash != dctk.TigerHashMust("L66Q4YVNAFWVS23X2HJIRA5ZJ7WXR3F26RSASFA") {
		panic(fmt.Errorf("wrong hash (5): %s", hash))
	}

	hash = dctk.TTHFromBytes([]byte(strings.Repeat("A", 1025)))
	if hash != dctk.TigerHashMust("PZMRYHGY6LTBEH63ZWAHDORHSYTLO4LEFUIKHWY") {
		panic(fmt.Errorf("wrong hash (6): %s", hash))
	}

	hash = dctk.TTHFromBytes([]byte(strings.Repeat("A", 10000)))
	if hash != dctk.TigerHashMust("UJUIOGYVALWRB56PRJEB6ZH3G4OLTELOEQ3UKMY") {
